...
`firecrawl` is a native Rust CLI over the Firecrawl v2 REST API, built to be driven by an LLM agent. It
replaced a .NET global tool (`Firecrawl.Console`) by Niels Bosma and preserves its interface: the same commands
and flags, YAML-first output, the same error envelope and exit codes, and adds secure native OS keystores and
rich multi-account isolation.

For the manual the *agent* reads, run `firecrawl agent-readme` — that text lives in `src/readme.rs` and is
the tool's actual interface for its main audience. This file is for the human or AI engineer editing the source.

## Commands

```bash
cargo build --release              # target/release/firecrawl
...
```bash
export FIRECRAWL_CONFIG_DIR=$(mktemp -d) FIRECRAWL_SECRET_STORE=plaintext FIRECRAWL_ALLOW_PLAINTEXT_STORE=1
```

| Variable | Effect |
| --- | --- |
| `FIRECRAWL_CONFIG_DIR` | Overrides the config and secrets storage directory |
| `FIRECRAWL_SECRET_STORE` | Forces a backend: `dpapi`, `keychain`, `libsecret`, `plaintext` |
| `FIRECRAWL_ALLOW_PLAINTEXT_STORE=1` | Permits the plaintext fallback where no keystore exists |
| `FIRECRAWL_API_URL` | Overrides the API base URL — how `tests/cli.rs` points at its mock |
| `FIRECRAWL_API_KEY` | Direct fallback API key (also supported via `--api-key`) |
...
src/
  ...
  commands/
    ...
    scrape.rs      scrape <URL> implementation
    crawl.rs       crawl start | status | cancel
    map.rs         map <URL>
  ...
  account.rs       --account or direct key resolution; identity probing
