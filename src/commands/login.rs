//! `firecrawl login`. Authenticates with Firecrawl via API key, opening the dashboard
...
const API_KEYS_URL: &str = "https://www.firecrawl.dev/app/api-keys";
...
pub fn run(args: LoginArgs) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists."))
            .fix(format!("Use --force to replace its key: firecrawl login {existing} --force")));
    }
...
fn prompt_login_key(name: &str, no_browser: bool) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API key given and no terminal to prompt on.")
            .fix(format!("pbpaste | firecrawl login {name} --api-key-stdin")));
    }

    eprintln!("To log in, copy or create an API key from Firecrawl:");
    ...
    loop {
        let key = rpassword::prompt_password(format!("Paste your Firecrawl API key for '{name}': "))
            .map_err(|e| Error::other("Could not read the API key.").detail(e.to_string()))?;
        let key = key.trim().to_string();
        if !key.is_empty() {
            return Ok(key);
        }
        eprintln!("Key cannot be empty. Paste your API key from {API_KEYS_URL}");
    }
}
