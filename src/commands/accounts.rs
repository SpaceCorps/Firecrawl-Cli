//! `firecrawl accounts add|list|test|remove`.
...
fn add(name: String, api_key: Option<String>, force: bool, no_verify: bool) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists.")).fix(format!(
            "Pick a different name, or replace its key: firecrawl accounts add {existing} --api-key <key> --force"
        )));
    }
    ...
    output::write(&obj! {
        ...
        "nextStep" => format!("firecrawl scrape https://example.com --account {name}"),
    });
    Ok(())
}
        ...
        if check {
            ...
            let status = match key {
                Some(k) => {
                    ...
                    match client.get("team/credit-usage") {
                        Ok(_) => "valid",
...
fn test(name: &str) -> Result<()> {
    ...
    let Some(api_key) = key.filter(|k| !k.trim().is_empty()) else {
        return Err(Error::new(ErrorCode::AuthRequired, format!("Account '{stored_name}' has no stored API key."))
            ...
            .fix(format!("firecrawl accounts add {stored_name} --api-key <key>")));
    };
...
fn remove(name: &str, yes: bool) -> Result<()> {
    ...
    output::write(&obj! {
        "status" => "removed",
        "name" => stored_name,
        "note" => "Key deleted from this machine. It has not been revoked on the Firecrawl platform.",
    });
    Ok(())
}
...
fn prompt_key(name: &str) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API key given and no terminal to prompt on.")
            .fix(format!("pbpaste | firecrawl accounts add {name} --api-key-stdin")));
    }

    let key = rpassword::prompt_password(format!("Enter Firecrawl API key for '{name}': "))
        .map_err(|e| Error::other("Could not read API key.").detail(e.to_string()))?;
    let key = key.trim().to_string();
    if key.is_empty() {
